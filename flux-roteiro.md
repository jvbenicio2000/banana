# Roteiro Flux.ai (Flux Copilot) — Placa de 6 Motores
...
O Flux busca por **MPN (código do fabricante)**, não pelo código LCSC — por isso
os prompts usam MPNs (os códigos LCSC ficam como referência para a hora de
exportar para a JLCPCB).
...
ELECTRICAL: Each motor 1–5A continuous @12V (stall up to ~3x). Design all 4
channels identically for the worst case: 10A continuous / 30A peak per channel.

PREFERENCES:
- Prefer SMD parts available on JLCPCB; give manufacturer part numbers (MPN).
- 2-layer or 4-layer, 2oz copper on power layers.
- Robust filtering, decoupling, reverse-polarity & overcurrent protection, and
  proper thermal design (copper pours + thermal vias / heatsink provisions).
...
sized for 10A continuous. Power chain: 12V input → fuse → reverse-polarity
P-MOSFET → TVS → EMI filter → bulk caps → (a) 12V power rail to H-bridges,
(b) buck 12V→5V (MP1584EN) → LDO AMS1117-3.3 for the ESP32. List the GPIO budget.
```

## ETAPA 2 — Alimentação e proteção

```
Build the power input and protection block: a 5.08mm screw terminal (or 2x 4mm
banana jacks) for 12V in, a fuse, a P-channel MOSFET reverse-polarity protection,
an SMBJ TVS, a common-mode choke + LC EMI filter, and bulk electrolytic
(1000uF/25V) + 100nF ceramics. Then add an MP1584EN buck to 5V and an AMS1117-3.3
LDO to 3.3V, each with proper input/output caps. Give MPNs.
```
...
```
Design ONE full H-bridge channel rated for 10A continuous / 30A peak from 12V:
- 4x IRF3205S N-MOSFETs (TO-263/D2PAK)
- 2x IR2104 half-bridge gate drivers with bootstrap diode + bootstrap cap
- gate resistors (22 ohm) and gate pull-downs (10k)
- local bulk cap (470-1000uF/25V) + 100nF decoupling
...
wire the two banana-jack pairs in PARALLEL on the board (the "2-by-2" pairing).
Keep these parallel power traces short and wide.
```

## ETAPA 6 — Proteção/sensoriamento (opcional, recomendado)

```
Add per-channel current sensing: a low-side shunt + INA180 amplifier feeding the
ESP32 ADC, for overcurrent protection in firmware. Add a power LED, a status LED,
and one LED per channel.
```

## ETAPA 7 — Layout e térmico

```
Now help with PCB layout: place the H-bridge MOSFETs over large copper pours with
dense thermal vias to a bottom plane; size copper for 10A continuous per channel.
Use 2oz copper, wide/short high-current traces (IPC-2221 for 10A, 30A peaks),
separate power ground (PGND) and signal ground (AGND) joined at a star point.
Leave room/holes for bolt-on heatsinks in case TO-220 MOSFETs are used. Run DRC.
```
...
- Para valores (bootstrap, snubber): *"reference the datasheet"* — ele lê datasheets.
...
| Roteamento de potência | **Fraca** | **Fazer manualmente** as trilhas de motor |
| Térmico / dissipadores | Fraca | Calcular (ver `dimensionamento/`) |

> Estratégia recomendada: Copilot faz o **esquemático** → você revisa
> **peças/footprints/pinos** → **layout de potência manual** → Copilot ajuda no
> roteamento de **sinal** e no **DRC**. Sempre prototipe e teste **1 canal**
> antes de produzir os 4.
